...
def validate_backend_value(value: str, field_name: str, _t: Callable[..., str] = _default_translate) -> None:
    ...
    只接受普通模型 provider id（``STANDARD_PROVIDER_REGISTRY`` 的 key 或 ``custom-`` 前缀）。legacy provider 名
    ...
    """
    if "/" not in value:
        if value in STANDARD_PROVIDER_REGISTRY:
    ...
    if provider_id not in STANDARD_PROVIDER_REGISTRY and not provider_id.startswith("custom-"):
