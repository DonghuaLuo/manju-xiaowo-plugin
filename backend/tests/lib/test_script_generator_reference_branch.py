...
from lib.script_splitting_templates import current_profile
...
def reference_project(tmp_path: Path) -> Path:
    ...
    (project_dir / "project.json").write_text(
        """{
          ...
          "script_splitting_template_id": "narration_legacy_reading_default",
...
async def test_script_generator_build_prompt_selects_reference_branch(reference_project: Path):
    ...
    profile = current_profile(gen.project_json)
    # reference 分支特征标签
    assert "ReferenceVideoScript" in prompt
    assert "references" in prompt
    assert "@[名称]" in prompt
    assert profile["id"] == "narration_legacy_reading_default"
    assert profile["hash"].startswith("sha256:")
...
@pytest.mark.asyncio
async def test_effective_generation_mode_ignores_episode_override(tmp_path: Path):
    """项目创建后模式固定：episode.generation_mode 不再覆盖 project.generation_mode。"""
    project_dir = tmp_path / "proj"
    project_dir.mkdir()
    import json as _j
    ...
    drafts = project_dir / "drafts" / "episode_1"
    drafts.mkdir(parents=True)
    (drafts / "step1_segments.md").write_text("E1S01 stub", encoding="utf-8")

    gen = ScriptGenerator(project_dir)
    prompt = await gen.build_prompt(episode=1)
    assert "ReferenceVideoScript" not in prompt
