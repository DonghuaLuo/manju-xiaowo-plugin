...
class TestRegistry:
    ...
    def test_video_caps_declaration_bindings(self):
        ...
        assert ENDPOINT_REGISTRY["dashscope-async-video"].video_caps_for_model("happyhorse-1.0-r2v").max_reference_images == 9
        assert ENDPOINT_REGISTRY["dashscope-async-video"].video_caps_for_model("wan2.7-r2v").max_reference_images == 5
    ...
    def test_media_type_groups(self):
        ...
        assert text_keys == {"openai-chat", "openai-responses", "gemini-generate"}
        assert image_keys == {
            "openai-images",
            "openai-images-generations",
            "openai-images-edits",
...
class TestHelpers:
    ...
    def test_list_endpoints_by_media_type(self):
        ...
        assert {s.key for s in text} == {"openai-chat", "openai-responses", "gemini-generate"}
...
class TestInferEndpoint:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("imagen-4", "openai", "gemini-image"),
            ("gemini-imagen-3", "openai", "gemini-image"),
            ...
            ("wan2.7-r2v", "openai", "dashscope-async-video"),
            ("wan2.7-image-pro", "openai", "openai-images"),
        ],
    )
...
def test_existing_image_endpoints_have_full_capabilities():
    ...
    assert ENDPOINT_REGISTRY["dashscope-image"].image_capabilities == full
