...
class TestBuildGridPrompt:
    ...
    def test_grid_allows_missing_cells_as_placeholders(self):
        scenes = [self._scene(f"S{i}", f"s{i}", f"a{i}") for i in range(1, 3)]
        prompt = build_grid_prompt(scenes=scenes, id_field="scene_id", rows=2, cols=2, style="anime")
        assert "剩余 2 个画格必须保持空占位" in prompt
        assert "格2（row2 col1）— 空占位" in prompt
    ...
    def test_grid_dimensions_in_header(self):
        ...
        prompt = build_grid_prompt(scenes=scenes, id_field="scene_id", rows=2, cols=2, style="realistic")
        assert "2×2" in prompt
        assert "格3（row2 col2）— 空占位" in prompt
    ...
    def test_no_placeholders_when_exact_fit(self):
        ...
        assert "补充镜头：" not in prompt

    def test_single_scene_fills_remaining_cells_with_placeholders(self):
        scenes = [self._scene("S1", "single scene", "single action")]
        prompt = build_grid_prompt(scenes=scenes, id_field="scene_id", rows=2, cols=2, style="realistic")
        assert "剩余 3 个画格必须保持空占位" in prompt
        assert "格1（row1 col2）— 空占位" in prompt
    ...
    def test_grid_3_panel_aspect_ratio(self):
        scenes = [self._scene(f"S{i}", f"s{i}", f"a{i}") for i in range(1, 4)]
        prompt = build_grid_prompt(
            scenes=scenes, id_field="scene_id", rows=2, cols=2, style="realistic", grid_aspect_ratio="9:16"
        )
        assert "整体图片比例：9:16" in prompt
        assert "每个画格比例：9:16" in prompt
