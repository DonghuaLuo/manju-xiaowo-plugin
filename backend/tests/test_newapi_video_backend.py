...
class TestNewAPIVideoBackend:
    ...
    async def test_create_retries_on_5xx(self, tmp_path: Path):
        """5xx HTTPStatusError 应通过 status_code 闸门重试。"""
    ...
    async def test_create_non_retryable_4xx_fails_fast(self, tmp_path: Path):
        """POST submit 阶段的确定性 4xx 不应重试到超时。"""
        ...
        with (
            patch("httpx.AsyncClient", return_value=mock_client),
            patch("lib.video_backends.newapi.download_video", AsyncMock()),
        ):
            from lib.video_backends.newapi import NewAPIVideoBackend
    ...
    async def test_poll_non_retryable_4xx_fails_fast(self, tmp_path: Path):
        """poll 阶段除 404/429 外的确定性 4xx 不应重试。"""
        create_resp = _make_response(200, {"task_id": "task-bad"})
        bad_resp = _make_response(401, {"error": "unauthorized"})
        bad_resp.raise_for_status = MagicMock(side_effect=_make_http_error(401, "unauthorized"))
