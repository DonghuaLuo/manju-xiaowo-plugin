...
@pytest.mark.asyncio
async def test_resume_other_exception_flips_pending_to_failed(tmp_path):
    """普通 resume 异常会让 task 进入 failed，ApiCall 也必须同步终态化。"""
    gen = _build_generator(tmp_path)
...
@pytest.mark.asyncio
async def test_resume_other_exception_propagates_finalize_exception(tmp_path):
    """普通异常分支同样不能吞 finalize 异常，否则仍可能留下 pending 缺口。"""
    gen = _build_generator(tmp_path)
    gen._video_backend = _FakeVideoBackend(raises=RuntimeError("network timeout"))
