...
@pytest.mark.asyncio
async def test_custom_active_credential_infers_auth_token_from_known_gateway_url(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session = AsyncMock()
    repo_mock = AsyncMock()
    ...
    result = await build_anthropic_env_dict(session)
    assert result["ANTHROPIC_API_KEY"] == ""
    assert result["ANTHROPIC_AUTH_TOKEN"] == "sk-custom"
    assert result["ANTHROPIC_BASE_URL"] == "https://api.deepseek.com/anthropic"


@pytest.mark.asyncio
async def test_custom_active_credential_keeps_api_key_for_unknown_gateway_url(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session = AsyncMock()
    repo_mock = AsyncMock()
    ...
    result = await build_anthropic_env_dict(session)
    assert result["ANTHROPIC_API_KEY"] == "sk-custom"
    assert result["ANTHROPIC_AUTH_TOKEN"] == ""
    assert result["ANTHROPIC_BASE_URL"] == "https://proxy.example.com/anthropic"
...
@pytest.mark.asyncio
async def test_no_active_credential_infers_auth_token_for_known_gateway_settings(
    monkeypatch: pytest.MonkeyPatch,
    base_url: str,
) -> None:
    session = AsyncMock()
...
@pytest.mark.asyncio
async def test_no_active_credential_prefers_explicit_auth_token_setting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session = AsyncMock()
    repo_mock = AsyncMock()
    ...
    result = await build_anthropic_env_dict(session)
    assert result["ANTHROPIC_API_KEY"] == ""
    assert result["ANTHROPIC_AUTH_TOKEN"] == "legacy-token"
    assert result["ANTHROPIC_BASE_URL"] == "https://proxy.example.com/anthropic"
