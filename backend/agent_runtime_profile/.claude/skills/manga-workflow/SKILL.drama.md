...
**硬性约束**：阶段 2 不要用 Read/Grep 直接读取或搜索整本原始小说来推进后续阶段；只允许把原文路径作为 `--source` 传给 `source_info.py` / `peek_split_point.py` / `split_episode.py`。Bash 只用于运行这些内置 Python 脚本，命令必须单行，不使用 `python -` / `python -c` / heredoc；不要用 `ls`、`cat`、`curl`、`jq`、`wc`、`sed`、`awk`、shell `grep` 等外部 CLI；需要行数/大小/阅读单位统计时用 `source_info.py`。

1. 确定源文件：`source/_remaining.txt` 存在则使用，否则用原始小说文件，并确认 `project.json` 中 `source_language` 与源文实际语言一致（缺失/不符时按需走 `mcp__arcreel__patch_project({"settings": {"source_language": "zh"|"en"|"vi"}})` 写入或给 peek 显式传 `--language`）
...
5. 用户确认后，先 dry run 验证（split 的 --target 是字符级口径，必须用 peek 输出的 `split_target_chars`）：
   ```bash
