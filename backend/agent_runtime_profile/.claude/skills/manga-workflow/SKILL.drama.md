---
name: manga-workflow
description: 将小说转换为短视频的端到端工作流编排器。当用户提到做视频、创建项目、继续项目、查看进度时必须使用此 skill。触发场景包括但不限于："帮我把小说做成视频"、"开个新项目"、"继续"、"下一步"、"看看项目进度"、"从头开始"、"拆集"、"自动跑完流程"等。即使用户只说了简短的"继续"或"下一步"，只要当前上下文涉及视频项目，就应该触发。不要用于单个资产生成（如只重画某张分镜图或只重新生成某个角色设计图——那些有专门的 skill）。
---
<!-- mode: drama -->

# 视频工作流编排

你（主 agent）是编排中枢。你**不直接**处理小说原文或生成剧本，而是：
1. 检测项目状态 → 2. 决定下一阶段 → 3. dispatch 合适的 subagent → 4. 展示结果 → 5. 获取用户确认 → 6. 循环

**核心约束**：
- 小说原文**永远不加载到主 agent context**，由 subagent 自行读取
- 每次 dispatch 只传**文件路径和关键参数**，不传大块内容
- 每个 subagent 完成一个聚焦任务就返回，主 agent 负责阶段间衔接
- Agent/Task 工具输入只传 `subagent_type`、`description`、`prompt`，不要传 `isolation` / `worktree`；prompt 不带项目绝对路径，session cwd 已经是项目根

> 三种生成模式（图生视频 / 宫格生视频 / 参考生视频）的数据路径与阶段分支详见 `.claude/references/generation-modes.md`。

---

## 阶段 0：项目设置

**重要**：项目目录的创建由插件工作台的项目创建流程触发 `ProjectManager.create_project()` 完成（包括所有子目录与 `project.json`、按 content_mode 物化对应的 agent profile）。**主 agent 不创建目录、不写入 project.json 初始字段**——session 启动时 cwd 已绑定到已存在的项目根。

### 新项目

1. 提示用户在插件工作台先创建项目，**创建时指定 content_mode**（narration / drama）；session 启动后 cwd 已绑定到对应项目根
2. 使用 Read 工具读取 `project.json`，确认 `title`、`content_mode`、`generation_mode` 字段（本 session 当前 content_mode 为 `drama`，创建后不可变更）
3. 若 `generation_mode` 未在创建时指定，AskUserQuestion 询问后由用户在插件工作台补齐（或由 mcp__arcreel__ 配置工具写入）
4. 请用户将小说文本放入 `source/`
5. **上传后自动生成项目概述**（synopsis、genre、theme、world_setting）

> 标准项目子目录由 `create_project()` 自动建好：`source/`、`scripts/`、`drafts/`、`characters/`、`scenes/`、`props/`、`storyboards/`、`grids/`、`videos/`、`reference_videos/`、`thumbnails/`、`output/`。

### 现有项目

1. session cwd 已经绑定到目标项目根
2. 通过 Read `project.json` + Glob 文件系统判定状态摘要
3. 从上次未完成的阶段继续

---

## 状态检测

进入工作流后，使用 Read 读取 `project.json`，使用 Glob 检查文件系统。Glob 的 `path` 指向项目根或已存在父目录，`pattern` 写待检查文件的相对子路径；遇到第一个缺失项即确定当前阶段：

1. characters / scenes / props 中**任一**为空（定义缺失）？ → **阶段 1**
2. 目标集 source/episode_{N}.txt 不存在？ → **阶段 2**
3. 目标集 drafts/ 中间文件不存在？ → **阶段 3**
   - generation_mode ∈ {storyboard, grid}: `drafts/episode_{N}/step1_normalized_script.md`
   - generation_mode == reference_video: `drafts/episode_{N}/step1_reference_units.md`
4. scripts/episode_{N}.json 不存在？ → **阶段 4**
5. 任一类资产仍有缺 sheet 项（character 缺 character_sheet / scene 缺 scene_sheet / prop 缺 prop_sheet）？ → **阶段 5**（三类并行）
6. **storyboard / grid 模式**：有片段 / 场景缺少分镜图？ → **阶段 6**（reference_video 模式跳过）
7. 有片段 / 场景 / unit 缺少视频？ → **阶段 7**
8. 全部完成 → 工作流结束，引导用户在插件工作台导出剪映草稿；若用户要求合并成片，可使用 `compose-video` 按当前剧本结构只合并现有片段

质量定位：角色 / 场景 / 道具母资产和宫格图保持高质量生成；分镜、视频、参考视频的批量生成与 Agent 自动生成默认使用快速版。用户明确要求精修时，可以对单镜头或批量镜头使用精修版；workflow 不主动把默认批量生成改成精修。

**确定目标集数**：如果用户未指定，找到最新的未完成集，或询问用户。

---

## 阶段间确认协议

**每个 subagent 返回后**，主 agent 执行：

1. **压缩摘要**：不要原样粘贴 subagent 返回内容；只用 1-3 句话或最多 5 条要点说明结果、关键数字和下一步
2. **获取确认**：使用 AskUserQuestion 提供选项：
   - **继续下一阶段**（推荐）
   - **重做此阶段**（附加修改要求后重新 dispatch）
   - **跳过此阶段**
3. **根据用户选择行动**

---

## 阶段 1：全局角色/场景/道具提取

**触发**：project.json 中 characters / scenes / props 中**任一**为空（定义缺失）

**dispatch `analyze-assets` subagent**：

```
项目名称：{project_name}
分析范围：{整部小说 / 用户指定的范围}
已有角色：{已有角色名列表，或"无"}
已有场景：{已有场景名列表，或"无"}
已有道具：{已有道具名列表，或"无"}

请按 analyze-assets 的分块读取规则读取项目原文，过滤已存在项，提取角色 / 场景 / 道具信息，写入 project.json，返回摘要。
```

---

## 阶段 2：分集规划

**触发**：目标集的 `source/episode_{N}.txt` 不存在

每次只切分当前需要制作的那一集。**主 agent 直接执行**（不 dispatch subagent）：

**硬性约束**：阶段 2 的源文件枚举、统计、切分点预览、分集写入和单集重跑清理固定调用 `mcp__arcreel__list_source_files` / `mcp__arcreel__source_info` / `mcp__arcreel__peek_split_point` / `mcp__arcreel__split_episode` / `mcp__arcreel__reset_episode_artifacts`。不要用 Read/Grep 直接读取或搜索整本原始小说来推进后续阶段。重跑 JSON / 分镜时不要删除 `source/episode_{N}.txt`；`reset_episode_artifacts` 默认只清 `scripts/episode_N.json` 和 `drafts/episode_N`，只有重新切分原文时才显式传 `include_source: true`。

1. 调用 `mcp__arcreel__list_source_files({})` 确定源文件：返回的 `preferred_source` 优先使用 `source/_remaining.txt`，否则使用原始小说文件；同时确认 `project.json` 中 `source_language` 与源文实际语言一致（缺失/不符时按需走 `mcp__arcreel__patch_project({"settings": {"source_language": "zh"|"en"|"vi"}})` 写入，或给 `peek_split_point` 显式传 `language`）
2. 解析目标字数（按 `source_language` 解读为阅读单位）：
   - 读 `project.json`，若已有 `episode_target_units` 字段 → 作为默认 target，向用户确认或调整
   - 字段缺失 + 用户当前对话已明确指定字数 → 使用对话值
   - 字段缺失 + 用户未指定 → AskUserQuestion 询问目标字数
   - 用户给出非临时性偏好时 → 调 `mcp__arcreel__patch_project({"settings": {"episode_target_units": N}})` 写回 project.json
3. 调用 `mcp__arcreel__source_info({"source": "{源文件}"})` 获取行数、大小、字符数和阅读单位统计，不返回正文
4. 调用 `mcp__arcreel__peek_split_point({"source": "{源文件}", "target": 目标阅读单位})` 展示切分点附近上下文（工具内部按 `source_language` 解读 target）
5. 分析 nearby_breakpoints，建议自然断点
6. 用户确认后，先 dry run 验证（split 的 `target` 是字符级口径，必须用 peek 输出的 `split_target_chars`）：
   `mcp__arcreel__split_episode({"source": "{源文件}", "episode": N, "target": split_target_chars, "anchor": "{锚点文本}", "dry_run": true})`
7. 确认无误后实际执行：
   `mcp__arcreel__split_episode({"source": "{源文件}", "episode": N, "target": split_target_chars, "anchor": "{锚点文本}"})`

---

## 阶段 3：单集预处理

**触发**：目标集的 drafts/ 中间文件不存在

根据 `effective_mode(project, episode)` 选择 subagent：

- `generation_mode == reference_video` → dispatch `split-reference-video-units`
- 否则 → dispatch `normalize-drama-script`

dispatch prompt 通用参数：项目名称、集数、本集小说文件路径（使用相对项目 cwd 的路径）。

（两个预处理 subagent 会自行读 project.json + 调用
`mcp__arcreel__get_video_capabilities({})`
拿到模型能力与用户偏好；主 agent 不需要预先注入角色/场景/道具列表或
`supported_durations` / `max_duration` / `max_reference_images` / `default_duration` 等数据。）

---

## 阶段 4：JSON 剧本生成

**触发**：scripts/episode_{N}.json 不存在

**dispatch `create-episode-script` subagent**：传入项目名称、集数。

---

## 阶段 5：资产设计（character / scene / prop 三类并行）

**前置条件**：三类资产的定义（characters / scenes / props）均已通过阶段 1 写入 project.json。若任一类定义为空（数组缺失），应回到阶段 1 补提取，而非停留在阶段 5。

**触发**：三类资产中任一类存在缺 sheet 项：
- character 缺 character_sheet
- scene 缺 scene_sheet
- prop 缺 prop_sheet

**调度规则（显式条件判断，按类型独立决定）**：

```
对于 type ∈ {character, scene, prop}:
  若该类存在缺 *_sheet 项 → dispatch 对应的 `generate-assets` subagent
  若该类均已齐全         → 跳过，不 dispatch

三类判断彼此独立，结果可能 dispatch 0~3 个 subagent。
所有 dispatch 的 subagent 返回后，合并为最多 5 条要点展示给用户，进入阶段间确认；不要原样粘贴各 subagent 的完整返回。
```

下面三个 dispatch 块是模板，只实例化满足上述条件的那几个：

### subagent — 角色设计

**触发**：有角色缺少 character_sheet

```
dispatch `generate-assets` subagent：
  任务类型：character
  项目名称：{project_name}
  待生成项：{缺失角色名列表}
  工具调用：
    mcp__arcreel__generate_assets({"type": "character"})
  验证方式：重新读取 project.json，检查对应角色的 character_sheet 字段
```

### subagent — 场景设计

**触发**：有场景缺少 scene_sheet

```
dispatch `generate-assets` subagent：
  任务类型：scene
  项目名称：{project_name}
  待生成项：{缺失场景名列表}
  工具调用：
    mcp__arcreel__generate_assets({"type": "scene"})
  验证方式：重新读取 project.json，检查对应场景的 scene_sheet 字段
```

### subagent — 道具设计

**触发**：有道具缺少 prop_sheet

```
dispatch `generate-assets` subagent：
  任务类型：prop
  项目名称：{project_name}
  待生成项：{缺失道具名列表}
  工具调用：
    mcp__arcreel__generate_assets({"type": "prop"})
  验证方式：重新读取 project.json，检查对应道具的 prop_sheet 字段
```

---

## 阶段 6：分镜图生成（仅 storyboard / grid 模式）

**触发**：有片段 / 场景缺少分镜图；**参考生视频模式跳过此阶段**

检查 `effective_mode(project, episode)`：

- `"storyboard"` → dispatch `generate-assets`，调 `mcp__arcreel__generate_storyboards`
- `"grid"` → dispatch `generate-assets`，调 `mcp__arcreel__generate_grid`
- `"reference_video"` → 不触发，直接跳到阶段 7

### storyboard 模式（默认）

**dispatch `generate-assets` subagent**：

```
dispatch `generate-assets` subagent：
  任务类型：storyboard
  项目名称：{project_name}
  工具调用：
    mcp__arcreel__generate_storyboards({"script": "episode_{N}.json"})
  验证方式：重新读取 scripts/episode_{N}.json，按当前脚本结构检查各片段 / 场景的 storyboard_image 字段
```

未显式传 `quality` 和 `selection_mode` 时工具默认生成快速版缺失分镜；workflow 不自动精修。若用户明确要求单镜头或批量精修，必须显式调用 `quality: "final"`，并用 `selection_mode: "selected"` + `segment_ids`、`selection_mode: "current_unrefined"` 或 `selection_mode: "current_all"` 指定范围。用户只说“全部精修 / 批量精修”但没说明范围时，先反问是只精修当前未精修，还是当前已有分镜全量重精修。

### grid 模式

**dispatch `generate-assets` subagent**：

```
dispatch `generate-assets` subagent：
  任务类型：storyboard
  项目名称：{project_name}
  工具调用：
    mcp__arcreel__generate_grid({"script": "episode_{N}.json"})
  验证方式：重新读取 scripts/episode_{N}.json，按当前脚本结构检查各片段 / 场景的 storyboard_image 字段
```

宫格图本身按高质量图生成；切出的分镜按快速版镜头使用。

---

## 阶段 7：视频生成

**触发**：有片段 / 场景 / unit 缺少视频

**dispatch `generate-assets` subagent**：

```
dispatch `generate-assets` subagent：
  任务类型：video
  项目名称：{project_name}
  工具调用：
    mcp__arcreel__generate_video_all({"script": "episode_{N}.json"})
  验证方式：重新读取 scripts/episode_{N}.json，按当前脚本结构检查各片段 / 场景 / unit 的 video_clip 字段
```

未显式传 `quality` 时工具默认生成快速版；workflow 默认用 `generate_video_all` 只补缺失或文件不存在的视频，不重跑已有当前视频。只有用户明确要求“重跑 / 重抽整集已有视频”时，才可调用 `generate_video_episode({"script": "...", "regenerate_existing": true})`。workflow 不自动传 `quality: "final"`。若用户明确要求整集 / 全部视频精修，必须显式传 `refine_scope: "current_unrefined"` 或 `refine_scope: "current_all"`；用户只说“全部精修 / 批量精修”但没说明范围时，先反问是只精修当前未精修，还是当前已有视频全量重精修。若用户要求合并本集视频，必须先按当前脚本结构确认每个片段 / 场景 / unit 都已有 `generated_assets.video_clip` 且文件存在；缺视频时提示缺失并停止，不得调用视频生成或精修生成来补齐。

---

## 灵活入口

工作流**不强制从头开始**。根据状态检测结果，自动从正确的阶段开始：

- "分析小说角色" → 只执行阶段 1
- "创建第2集剧本" → 从阶段 2 开始（如果角色已有）
- "继续" → 状态检测找到第一个缺失项
- 指定具体阶段（如"生成分镜图"）→ 直接跳到该阶段

---

## 数据分层

- 角色 / 场景 / 道具完整定义**只存 project.json**，剧本中仅引用名称
- 统计字段（scenes_count、status、progress）**读时计算**，不存储
- 剧集元数据在剧本保存时**写时同步**
