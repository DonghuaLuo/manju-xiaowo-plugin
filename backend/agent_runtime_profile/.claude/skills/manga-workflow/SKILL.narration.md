...
**硬性约束**：阶段 2 不要用 Read/Grep 直接读取或搜索整本原始小说来推进后续阶段；只允许把原文路径作为 `--source` 传给 `source_info.py` / `peek_split_point.py` / `split_episode.py`。Bash 命令必须单行，不使用 `python -` / `python -c` / heredoc；需要行数/大小/阅读单位统计时用 `source_info.py`。
