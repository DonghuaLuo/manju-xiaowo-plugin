...
**重要：生成视频固定调用下列 MCP 工具入队；此 skill 没有本地脚本入口。**

通过 MCP 工具入队：

| 操作 | 工具 |
|------|------|
| 补齐缺失视频（默认 / workflow 自动） | `mcp__arcreel__generate_video_all({"script": "episode_1.json"})` |
| 整集生成（默认跳过已有当前视频） | `mcp__arcreel__generate_video_episode({"script": "episode_1.json"})` |
| 断点续传 | `mcp__arcreel__generate_video_episode({"script": "episode_1.json", "resume": true})` |
| 整集快速重抽已有视频（用户明确要求重跑整集时） | `mcp__arcreel__generate_video_episode({"script": "episode_1.json", "regenerate_existing": true})` |
...
| 精修当前未精修视频 | `mcp__arcreel__generate_video_episode({"script": "episode_1.json", "quality": "final", "refine_scope": "current_unrefined"})` |
| 当前已有视频全量重精修 | `mcp__arcreel__generate_video_episode({"script": "episode_1.json", "quality": "final", "refine_scope": "current_all"})` |

> 所有任务一次性提交到生成队列，由 Worker 按 per-provider 并发配置自动调度。
> 集号从 script 顶层 `episode` 或文件名推导，无需手动传。
> `reference_video` 模式下 `scene_id` / `scene_ids` 视为 `video_units[].unit_id`；整集入口才会生成全体 unit。

## 质量档位规则

- `draft` = **快速版**：所有整集批量、全部待处理、批量自选、Agent 自动生成默认都用快速版；默认补缺只处理缺失或文件不存在的视频，不重跑已有当前视频。
- `final` = **精修版**：只在用户明确要求精修时使用；可以按用户指令对单个镜头 / unit 或一批镜头 / unit 精修，自动工作流不默认精修。
- 整集 / 全部 / 批量精修必须显式传 `refine_scope`：`current_unrefined` 只精修当前未精修/快速版视频，`current_all` 会把当前已有视频全量重精修（包括已精修项）。两者都只看当前版本，历史版本不算。
- 用户只说"全部精修"、"批量精修"但没有说明范围时，必须先反问是 `current_unrefined` 还是 `current_all`，不得直接猜测调用。
- 不要为了"合并成片"、"导出剪映草稿"或"本集完成"把快速版视频重新生成为精修版。

## 工作流程

1. **加载项目和剧本** — 确认所有场景都有 `storyboard_image`
2. **生成视频** — MCP 工具自动构建 Prompt、调用 API、保存 checkpoint；未指定质量时默认快速版
