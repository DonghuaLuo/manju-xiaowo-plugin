...
当前 session cwd 已经是项目根，但 Read / Glob / Grep 的路径参数仍使用绝对路径。不要用 Bash 做文件浏览、统计或网络请求，不要拼 `/d/...` 这类类 Unix 盘符路径，不要用 `ls`、`cat`、`curl`、`jq`、`wc`、`sed`、`awk`、shell `grep`、`python -c` 或临时 Python 脚本读取小说；如需行数/大小/阅读单位统计，执行 `python .claude/skills/manage-project/scripts/source_info.py --source source/xxx.txt`。

读取规则：
- 先用 Glob 定位 `source/` 下的文本文件；若已有 `source/episode_{N}.txt`，优先读取分集文件。
- 大文件只允许用 Read 分块读取，单次 `limit <= 200` 行；不要一次性 Read 整本原文，也不要传 `limit > 200`。
- 整部小说分析时，按文件顺序分批读取必要窗口并自行归纳，不把原文大段复制到最终摘要。
- Grep 只用于少量明确名称复核，参数使用 SDK 字段名：`-n`、`-C`、`-A`、`-B`、`-i`，不要写成 `n` / `C`。
...
- 严禁用 Write/Edit/Bash/PowerShell 直接改 project.json——只能走 patch_project 工具
