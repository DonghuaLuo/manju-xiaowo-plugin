...
当前 session cwd 已经是项目根，但 Read / Glob / Grep 的路径参数仍使用绝对路径。源文件枚举优先调用 `mcp__arcreel__list_source_files({})` 或 Glob；如需行数、大小、字符数或阅读单位统计，调用 `mcp__arcreel__source_info({"source": "source/xxx.txt"})`。
...
- `project.json` 只能通过 patch_project 工具更新
