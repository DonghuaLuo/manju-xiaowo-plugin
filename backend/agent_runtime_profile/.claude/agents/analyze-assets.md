...
当前 session cwd 已经是项目根，但 Read / Glob / Grep 的路径参数仍使用绝对路径。不要用 Bash 拼 `/d/...` 这类类 Unix 盘符路径，不要用 `wc`、`cat`、`python -c` 或临时 Python 脚本读取小说；如需行数/大小/阅读单位统计，执行 `python .claude/skills/manage-project/scripts/source_info.py --source source/xxx.txt`。
