...
- **先给结论**：不要先铺背景、过程、长清单或大段引用；需要说明时只列关键原因。
- **禁止信息倾倒**：不要把工具输出、subagent 摘要、文件内容原样大段粘给用户；压缩成结论 + 关键数字 + 下一步。
- **按需展开**：只有用户明确要求"详细说明 / 全部列出 / 完整报告 / 展开"时，才输出长答案。
- **状态汇报上限**：最多 5 条要点；完成工作后只说改了什么、结果如何、是否需要用户决策。
...
- **编辑项目 JSON**：修改剧本（`scripts/*.json`）或角色/场景/道具（`project.json`）**一律走 `mcp__arcreel__*` 编辑工具**——剧本改字段用 `patch_episode_script`，增/删/拆分镜用 `insert_segment` / `remove_segment` / `split_segment`，角色/场景/道具与 settings 用 `patch_project`。这两类 JSON 受 sandbox `denyWrite` 与 PreToolUse hook 双层保护。**改 prompt 必重生**：用 `patch_episode_script` 改了某分镜的 `image_prompt` / `video_prompt` 后，工具不会自动作废旧图/视频，必须紧接着调对应生成工具重新生成该分镜，否则会留下「新 prompt + 旧画面」的陈旧。
- **项目文件操作**：源文件枚举、统计、分集切分、单集重跑清理一律走 `mcp__arcreel__list_source_files` / `source_info` / `peek_split_point` / `split_episode` / `reset_episode_artifacts`。重跑 JSON / 分镜默认保留 `source/episode_N.txt`，只有重新切分原文时才显式删除分集源文。JSON 剧本摘要优先使用 `mcp__arcreel__generate_episode_script` 返回值或 Read 工具。
...
- **项目内路径**：MCP 工具参数使用**相对项目根 cwd** 的路径，例如：
...
- **源文件统计**：需要行数、大小、字符数或阅读单位时，调用 `mcp__arcreel__source_info({"source": "source/xxx.txt"})`
- **Subagent dispatch**：Agent/Task 工具只传 `subagent_type`、`description`、`prompt`；不要传 `isolation` / `worktree`，prompt 里也不要注入项目绝对路径。subagent 的 cwd 已经是当前项目根。
- **严禁**在工具参数中出现 `projects/{...}/` 前缀；该前缀仅用于文档说明项目目录结构，**不可直接作为参数传给任何工具**
- skill 脚本内部已加 cwd 校验，cwd 漂离当前项目目录时会直接拒绝执行
- **未分集时**：如果 `source/episode_{N}.txt` 不存在，必须先走 `manga-workflow` 阶段 2 的 `mcp__arcreel__peek_split_point` / `mcp__arcreel__split_episode` 生成单集文件；不要直接 Read/Grep 整本原文推进后续阶段
...
| **Skill（在 subagent 内调用）** | MCP 工具编排 → API 调用、文件生成 | generate-script、generate-assets、compose-video |
...
| compose-video | `/compose-video` | 调 `mcp__arcreel__compose_video` 单集合并现有视频片段（可选 BGM / 转场；不做片头片尾或多集合并） |
...
8. **视频生成** → dispatch `generate-assets` subagent（MCP 工具自动按 video_units/segments/scenes 分派）

工作流支持**灵活入口**：状态检测自动定位到第一个未完成的阶段，支持中断后恢复。
视频生成完成后，用户可在插件工作台导出为剪映草稿。
...
> 下面的目录树仅为说明用途，agent session 的 cwd 已在项目根。MCP 工具参数使用相对 cwd 的路径（如 `source/`、`scripts/`）；**Read / Edit / Write / Glob / Grep** 的 `file_path` 仍按上文"路径规范"要求使用**绝对路径**。无论哪种工具都不可带 `projects/{项目名}/` 前缀。
