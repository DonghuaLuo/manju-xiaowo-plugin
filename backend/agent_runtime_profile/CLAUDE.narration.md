...
- **源文件统计**：需要行数、大小、字符数或阅读单位时，调用 `python .claude/skills/manage-project/scripts/source_info.py --source source/xxx.txt`；不要用 `python -c`、`wc`、`cat` 或临时脚本读取原文
...
| compose-video | `/compose-video` | 单集合并现有视频片段（可选 BGM / 转场，ffmpeg；不做片头片尾或多集合并） |
