...
同步评估时默认采用“积极采纳、按 manju 改造”的口径。只要 ArcReel 上游更新满足以下条件，就应进入“建议同步”或“需要改造后同步”，不要轻易列为“需要用户决定”：

- 不破坏 manju 当前本地功能。
- 不破坏小蜗桌面插件架构、插件 SDK 生命周期、前后端通信方式、本地文件能力和任务事件链路。
- 不覆盖或回退 manju 已经做得更适合桌面插件的本地优化。
- 不回退 manju 已经针对报错、边界条件、任务状态、供应商能力或前端用户体验做过的修复。
- 不让前端用户体验退化，例如不移除已有提示、校验、i18n、桌面文件选择、拖拽、任务状态展示或本地额外配置能力。
- 上游思路能适配到 manju 现有模块、store、API 封装、后端 resolver、任务队列、agent runtime 或 i18n 体系。

如果 ArcReel 的更新方向有价值，但直接复制会破坏 manju 桌面插件边界，应提取上游意图并改造成 manju 版本，分类为“需要改造后同步”，而不是默认丢给用户判断。
...
如果用户明确说“全部更新”“可以全部采纳”“只要不破坏 manju 本地功能和桌面插件架构就采纳”等同类指令，则按上面的默认采纳边界执行：可适配、不退化、不回退本地修复的更新都应采纳或改造后采纳；但仍不得同步与插件运行无关的 Web 部署、CI、仓库治理、浏览器登录态或外部 agent 配置。
...
- ArcReel 修复的问题也存在于 manju，但 manju 对应模块已经有本地增强；此时应保留 manju 更强实现，只移植缺失语义、测试和边界处理。
...
只在以下情况列为“需要用户决定”。不要把“需要适配”“需要测试”“需要按 manju 改写”的工程问题误列为用户决定：
...
- 需要决定是否启用新的供应商、协议、定价策略或多字段凭证模型。
- 需要决定是否新增面向用户的配置入口，而不是只同步后端能力默认值。
...
- 如果用户要求“帮我同步 / 开始修改”，实现分类为“建议同步”和“需要改造后同步”的项；后者必须按 manju 桌面插件架构改写，不机械复制。
- 分类为“需要用户决定”的项才先问用户，不要为了普通工程适配频繁打断。
...
同步“建议同步”和“需要改造后同步”的内容；改造后同步项要按 manju 桌面插件结构落地。
