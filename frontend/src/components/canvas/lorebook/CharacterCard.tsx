...
import { isDesktopFileRef, pickDesktopFile, type UploadFileInput } from "@/utils/desktop-file";
import type { Character } from "@/types";

interface CharacterSavePayload {
  description: string;
  voiceStyle: string;
  referenceFile?: UploadFileInput | null;
}
...
export function CharacterCard({
  ...
  const [referenceFile, setReferenceFile] = useState<UploadFileInput | null>(null);
  ...
  const handleReferenceChange = async () => {
    ...
    setReferencePreview((prev) => {
      if (prev?.startsWith("blob:")) URL.revokeObjectURL(prev);
      return isDesktopFileRef(file) ? (file.previewUrl ?? null) : URL.createObjectURL(file);
    });
  };
    ...
    >
      ...
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <div className="flex shrink-0 items-center gap-0.5">
          <button
            type="button"
            onClick={() => void handleSheetUpload()}
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => void handleReferenceChange()}
