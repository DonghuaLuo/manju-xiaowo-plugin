...
export function SourceFilesPage({ projectName }: SourceFilesPageProps) {
    ...
    async (file: UploadFileInput) => {
      ...
      if (!isSupportedSourceFileName(filename)) {
        useAppStore
          .getState()
          .pushToast(
            tRef.current("dashboard:source_unsupported_extension", { filename }),
            "error",
          );
        return;
      }
      ...
      const tryUpload = async (
        ...
        const filename = res.filename ?? getUploadFileName(file);
        ...
        if (err instanceof ConflictError) {
          ...
          if (projectNameRef.current !== targetProject) {
            ...
            useAppStore
              .getState()
              .pushToast(
                tRef.current("dashboard:upload_aborted_project_changed", { filename }),
                "error",
              );
            return;
          }
  ...
  const handlePickSourceFile = useCallback(async () => {
    if (uploadInFlightRef.current) return;
